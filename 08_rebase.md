...
はい、トピックブランチ(今回で言えば unify_styles)を切って選択しましたか？ もうコマンドはここには書かないので自分でやってみてください。
...
まずは、unify_styles を「巻き戻し」して、master からあたらしくブランチを切り直します。そこから、unify_styles の各コミットで行った変更を、master が指しているコミットに対して適用しなおしているわけです。つまり、「適用中：作業途中だがとりあえずコミットしておく」が意味しているのは、unify_styles ブランチで行った「作業途中だがとりあえずコミットしておく」のコミットでの変更内容を、再度 master から適用しなおしているよ、という意味だったのですね！
...
でも、ちょっと待ってください。今回はたまたま「変更のやり直し」が機械的に可能だったけれど、もしも Git さんが「やり直し」をしている間に、「機械的に変更の適用」ができない事態に陥ったらどうなるのでしょう？ そう、コンフリクトですね。
